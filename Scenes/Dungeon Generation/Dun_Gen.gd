...
@export var start : bool = false : set = _set_start

func _set_start(value:bool)->void:
	if Engine.is_editor_hint():
		_generate()

@export_range(0, 1) var survival_chance : float = 0.25
@export var border_size : int = 20 : set = _set_border_size
...
func _set_border_size(value : int)->void:
	border_size = value
	if Engine.is_editor_hint():
		_visualize_border()
...
func _generate():
	...
	_make_hallways(hallway_graph)

func _make_room(recursion:int):
	if !recursion > 0:
		return
...
func _make_hallways(hallway_graph:AStar2D):
